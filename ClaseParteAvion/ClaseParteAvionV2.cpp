...
public:
    ...
    const char* getNomParte() const { return nomParte; }
    float getVidaUtil() const { return vidaUtil; }
    float getTiempoUso() const { return TiempoUso; }
    float getTiempoRest() const { return TiempoRest; }

    void setClave(int c) { clave = c; }
    void setNomParte(const char* nombre) { strcpy(nomParte, nombre); }
    void setVidaUtil(float vida) { vidaUtil = vida; }
    void setTiempoUso(float uso) { TiempoUso = uso; }
    void setTiempoRest(float rest) { TiempoRest = rest; }
}; // Fin clase Parte
